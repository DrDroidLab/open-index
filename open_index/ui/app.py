...
def _current_url():
    """The URL the browser is on, as reported over the websocket.

    Absent on older Streamlit versions and in bare script runs, so callers must
    tolerate None.
    """
    try:
...
def _select_brain():
    """The brain this page shows, chosen solely by the URL path.

    With OPEN_INDEX_BRAINS_ROOT set, one process serves every brain under it —
    the alternative is a process, and a ~250MB embedding model, per brain. The
    index is the first path segment, so /<name> serves <name>. There is no
    in-page switcher on purpose: the URL is the selector, which keeps the
    address bar and the screen in agreement and makes every view shareable.
    """
    ...
    chosen = view.brain_from_url(_current_url(), list(brains))
    # The directory name is also the URL segment, so it is what builds this
    # index's endpoint — the brain's configured name may differ from it.
    return _open_brain(brains[chosen]), chosen
...
def render_how_to_use(brain: Brain, url_name: Optional[str] = None) -> None:
    ...
    """
    summary = view.summarize(brain)
    # Derived from the URL this page is on, so it is right for *this* index and
    # needs no configuration. Falls back to an explicitly configured URL for a
    # single-brain deployment, where the page path carries no index name.
    mcp_url = (view.mcp_url_for(_current_url(), url_name)
               or os.environ.get("OPEN_INDEX_PUBLIC_URL", ""))
...
def main() -> None:
    ...
    with tab_help:
        render_how_to_use(brain, url_name)
    with tab_schema:
        render_schema(brain)
    with tab_explore:
