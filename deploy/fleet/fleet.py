...
def render_compose(config: dict) -> str:
    ...
    for entry in config["indexes"]:
        ...
        token_env = (f"      OPEN_INDEX_TOKEN: ${{{env_var(name)}}}\n"
                     if config.get("auth", True) else "")
        services.append(f"""
  ...
  brain-{name}:
    ...
    command: {command}
    environment:
{token_env}      OPEN_INDEX_SEARCH_BACKEND: opensearch
...
""".rstrip("\n"))
  ...
  ui-{name}:
    ...
    environment:
      ...
      OPEN_INDEX_PUBLIC_URL: {base}/{name}/mcp
      OPEN_INDEX_READ_ONLY: "{str(bool(entry.get("read_only"))).lower()}"
...
""".rstrip("\n"))
    ...
    caddy_deps = "".join(
        f"      - brain-{e['name']}\n"
        + (f"      - ui-{e['name']}\n" if e.get("ui", True) else "")
        for e in config["indexes"]) or "      []\n"
    caddy_env = ""
    if config.get("auth", True):
        for e in config["indexes"]:
            if e.get("ui", True):
                var = "UI_HASH_" + e["name"].upper().replace("-", "_")
                caddy_env += f"      {var}: ${{{var}}}\n"
    caddy_env = caddy_env or "      PLACEHOLDER: \"\"\n"

    return f"""# GENERATED BY fleet.py — do not edit. Change indexes.yml and re-run `fleet.py up`.
#
# One shared OpenSearch cluster, one brain container per index, one Caddy in
# front doing path routing and TLS.
...
{"".join(services)}
  ...
  caddy:
    ...
    ports:
      ...
      - "443:443"
    environment:
{caddy_env}    depends_on:
{caddy_deps}    restart: unless-stopped
...
def render_caddyfile(config: dict) -> str:
    """Caddy config: one site, one route per index, automatic HTTPS.

    Caddy rather than nginx because it obtains and renews the TLS certificate
    itself from Let's Encrypt — with a real hostname (nip.io resolves any
    <ip>.nip.io to that ip) that removes the whole certbot dance. It also
    upgrades websockets and streams SSE without extra directives, which the MCP
    transport and Streamlit both need.
    """
    ...
    listing = {}
    for e in config["indexes"]:
        item = {"mcp": f"{base}/{e['name']}/mcp",
                "description": e.get("description", ""),
                "read_only": bool(e.get("read_only"))}
        if e.get("ui", True):
            item["ui"] = f"{base}/{e['name']}/ui"
        listing[e["name"]] = item
    ...
    for entry in config["indexes"]:
        ...
        if auth:
            guard = (f"\n\t\tbasicauth {{\n"
                     f"\t\t\t{name} {{env.UI_HASH_{name.upper().replace('-', '_')}}}\n"
                     f"\t\t}}")
        if entry.get("ui", True):
            blocks.append(f"""
	# {name} — explorer UI. No prefix stripping: Streamlit runs with
	# baseUrlPath={name}/ui and expects the full path.{guard}
	handle /{name}/ui* {{
		reverse_proxy ui-{name}:8501
	}}""")
        blocks.append(f"""
	# {name} — MCP endpoint. handle_path strips the /{name} prefix so the
	# brain receives /mcp.
	handle_path /{name}/* {{
		reverse_proxy brain-{name}:8080 {{
...
{"".join(blocks)}
	...
	# Directory of what this host serves. Names and URLs only.
	handle {{
