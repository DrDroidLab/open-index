...
def render_caddyfile(config: dict) -> str:
    ...
    # One explicit handle per index. The MCP process routes /<name>/mcp itself,
    # so the path is passed through rather than stripped.
    index_routes = "".join(
        f"\n\t\thandle /{e['name']}/* {{"
...
{host} {{
	# `route` preserves this order exactly. Without it Caddy sorts handle blocks
	# by matcher specificity, and the catch-all directory can win over the index
	# routes — every MCP request then returns the directory listing.
	route {{
...
def cmd_up(config: dict) -> None:
    ...
    # The Caddyfile is bind-mounted: rewriting it changes nothing until Caddy
    # re-reads it, and compose will not recreate a container whose spec is
    # unchanged. Reload explicitly so a new index actually gets a route.
    run(compose_cmd() + ["exec", "-T", "caddy",
